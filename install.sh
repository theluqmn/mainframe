...
echo ""
echo "Welcome to a basic CLI-based somewhat interactive installer for Mainframe, a web OS server management software."
echo ""
...
read -p "(2/3) Where to create a data folder? (./data): " data_folder
...
echo ""
echo "This will be required to access the web OS. The address is either the domain that you have pointed to the server, or the static public IPv4 address."
read -p "Enter an address (example.com): " hostname
if [ -z "$hostname" ]; then
    hostname="127.0.0.1"
fi
